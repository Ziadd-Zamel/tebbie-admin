...
export default function MultiSearchSelectorSelectAll({
  options,
  onChange,
  selectedValues,
  placeholder,
  ...
  const handleSelectAll = () => {
    setFormState((prev) => {
      ...
      const areAllSelected = prev.user_ids.length === allUserIds.length;
      return {
        ...prev,
        user_ids: areAllSelected ? [] : allUserIds,
      };
    });
  };
        ...
        <div className="absolute w-full mt-1 bg-white border border-gray-300 rounded-md shadow-lg max-h-80 overflow-y-auto z-10">
          <label className="flex items-center mt-2 gap-2">
            <input
              ...
              className=" w-4 h-4 ms-3 text-blue-600 border-gray-300 rounded"
            />
