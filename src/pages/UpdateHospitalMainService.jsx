...
const UpdateHospitalMainService = () => {
  ...
  const { data: mainServicesData, isLoading: mainServicesLoading } = useQuery({
  ...
  const { mutate: handleSubmit, isPending } = useMutation({
    mutationFn: (data) => updateHospitalMainService({ ...data, token, id }),
    onSuccess: () => {
      queryClient.invalidateQueries(["hospital-main-services", token]);
      toast.success(t("تم تحديث الخدمة الرئيسية بنجاح"));
      navigate("/hospital-services");
    },
    onError: (error) => {
      toast.error(error.message || t("فشل في تحديث الخدمة الرئيسية"));
    },
  });
  ...
  useEffect(() => {
    if (mainServicesData && id) {
      ...
      if (mainService) {
        setFormData({
          hospital_id: mainService.hospital_id || "",
          name: mainService.name || "",
          status: mainService.status || "active",
        });
      }
    }
  }, [mainServicesData, id]);
  ...
  if (mainServicesLoading || hospitalsLoading) return <Loader />;
      ...
      <div className="bg-white rounded-3xl md:p-8 p-4 w-full">
        ...
        >
          ...
          <div className="flex justify-center gap-4 mt-8">
            ...
            >
              {isPending ? t("جاري التحديث...") : t("تحديث")}
            </button>
          </div>
        </form>
      </div>
...
export default UpdateHospitalMainService;
