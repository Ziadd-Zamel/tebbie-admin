...
const DoctorSchema = Yup.object().shape({
  ...
  bio: Yup.string().required('bio_required'),
...
const DoctorForm = ({ initialData, mode = 'add', isLoading, error }) => {
  ...
  const initialValues = initialData ? {
    ...
    hospital_ids: initialData.hospitals?.map(hospital => hospital.id) || [],
          ...
          <Form className="w-auto rounded-3xl bg-white lg:p-12 p-6 shadow-lg flex justify-center flex-col mx-auto">
            ...
            <div className="lg:flex w-full justify-center">
              <div className="px-3 my-6 md:mb-0 w-full">
                ...
                <label className="block text-md almarai-semibold mb-4" htmlFor="name"> <span className="text-red-500">*</span> {t('name')}</label>
                <Field
                  type="text"
                  name="name"
                  id="name"
                ...
                />
                <ErrorMessage name="name" component="div" className="!text-red-700 my-2" render={msg => t(msg)} />
              </div>

              <div className="px-3 my-6 md:mb-0 w-full">
                <label className="block text-md almarai-semibold mb-4" htmlFor="bio"> <span className="text-red-500">*</span>  {t('bio')}</label>
                <Field
                  type="text"
                  name="bio"
                  id="bio"
                  ...
                  className="border border-gray-300 rounded-lg py-2 px-4 bg-[#F7F8FA] h-[50px] focus:outline-none focus:border-primary w-full"
                />
                <ErrorMessage name="bio" component="div" className="!text-red-700 mt-1" render={msg => t(msg)} />
              </div>
            </div>
            ...
            <div className="lg:flex w-full justify-center">
              <div className="px-3 my-6 md:mb-0 w-full">
                <label className="block text-md almarai-semibold mb-4" htmlFor="email"> <span className="text-red-500">*</span>  {t('email')}</label>
                <Field
                  type="text"
                  name="email"
                  id="email"
                  ...
                  className="border border-gray-300 rounded-lg py-2 px-4 bg-[#F7F8FA] h-[50px] focus:outline-none focus:border-primary w-full"
                />
                <ErrorMessage name="email" component="div" className="text-red-700 mt-1" render={msg => t(msg)} />
              </div>

              <div className="px-3 my-6 md:mb-0 w-full">
                <label className="block text-md almarai-semibold mb-4" htmlFor="address"> <span className="text-red-500">*</span>  {t('address')}</label>
                <Field
                  type="text"
                  name="address"
                  id="address"
                  ...
                  className="border border-gray-300 rounded-lg py-2 px-4 bg-[#F7F8FA] h-[50px] focus:outline-none focus:border-primary w-full"
                />
                <ErrorMessage name="address" component="div" className="text-red-700 mt-1" render={msg => t(msg)} />
              </div>
            </div>

            <div className="lg:flex w-full justify-center">
              <div className="px-3 my-6 md:mb-0 w-full">
                <label className="block text-md almarai-semibold mb-4" htmlFor="phone"> <span className="text-red-500">*</span>  {t('phone')}</label>
                <Field
                  type="text"
                  name="phone"
                  id="phone"
                  ...
                  className="border border-gray-300 rounded-lg py-2 px-4 bg-[#F7F8FA] h-[50px] focus:outline-none focus:border-primary w-full"
                />
                <ErrorMessage name="phone" component="div" className="text-red-700 mt-1" render={msg => t(msg)} />
              </div>
              ...
              <label className="block text-md almarai-semibold mb-4" htmlFor="job_title"> <span className="text-red-500">*</span> {t('jobtitle')}</label>
                <Field
                  ...
                  className="border border-gray-300 rounded-lg py-2 px-4 bg-[#F7F8FA] h-[50px] focus:outline-none focus:border-primary w-full"
                />
                <ErrorMessage name="job_title" component="div" className="text-red-700 mt-1" render={msg => t(msg)} />
              </div>
            </div>
            ...
            <div className="lg:flex w-full justify-center">
              ...
              <label className="block text-md almarai-semibold mb-4"htmlFor="specialization_id"> <span className="text-red-500">*</span>{t('specializations')}</label>
                ...
                <ErrorMessage name="specialization_id" component="div" className="text-red-700 mt-1" render={msg => t(msg)} />
              </div>

              <div className="px-3 my-6 md:mb-0 w-full">
                <label className="block text-md almarai-semibold mb-4" htmlFor="hospital_ids">{t('hospitals')}</label>
                {hospitalDataLoading ? (
                  <div className="text-gray-500">Loading...</div>
                ) : (
                  <MultiSelectDropdown
                    ...
                    handleDoctorChange={(selectedHospitalIds) => setFieldValue('hospital_ids', selectedHospitalIds)}
                    translation="hospitals"
                  />
                )}
                <ErrorMessage name="hospital_ids" component="div" className="text-red-700 mt-1" render={msg => t(msg)} />
              </div>
            </div>
            ...
            <div className="flex gap-2">
              <div className="text-2xl font-medium gap-2 flex justify-start items-center my-4 px-4">
                ...
                <Field
                  ...
                  onChange={(e) => setFieldValue('is_visitor', e.target.checked ? 'yes' : 'no')}
                />
              </div>
              <div className="text-2xl font-medium gap-2 flex justify-start items-center my-4 px-4">
                ...
                <Field
                  ...
                  onChange={(e) => setFieldValue('isAbleToCancel', e.target.checked ? 'yes' : 'no')}
                />
              </div>
            </div>
            ...
            {mutation.error && (
              <div className="w-full flex justify-start">
                <ul className="text-red-700 px-4 py-3 rounded mb-4 list-disc ml-4">
                  {(Array.isArray(mutation.error) ? mutation.error : [mutation.error.message || 'حدث خطأ غير متوقع'])
                    .map((error, index) => (
